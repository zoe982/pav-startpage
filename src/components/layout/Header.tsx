      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {isAuthenticated && (
            <nav className="hidden items-center gap-6 sm:flex">
              ...
              {user?.isAdmin && (
                <Link
                  to="/admin"
                  className="text-sm font-medium text-pav-terra motion-standard hover:text-pav-terra-hover"
                >
                  Admin
                </Link>
              )}
        ...
        {isAuthenticated && user && (
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={logout}
              className="rounded-md px-3 py-1.5 text-sm text-on-surface-variant motion-standard hover:bg-pav-tan/20 hover:text-pav-blue"
            >
              Sign out
            </button>
          </div>
